...
import { QuestionKeyNav } from "@/components/QuestionKeyNav";
...
export default async function QuestionPage({ params }: { params: Promise<{ id:string }> }) {
  ...
  return <div className="container"><div className="page-intro"><Link href="/questions" className="text-link">← Back to questions</Link><div className="eyebrow" style={{marginTop:26}}>Guided practice</div><h1>{questionLabel(q)}</h1></div>
    ...
    <QuestionKeyNav previousId={previous?.id} nextId={next?.id} /><nav className="question-nav" aria-label="Question navigation">{previous ? <Link className="button secondary" href={`/questions/${previous.id}`}>← {questionLabel(previous)}</Link> : <span />}{next && <Link className="button" href={`/questions/${next.id}`}>{questionLabel(next)} →</Link>}</nav></div>;
}
